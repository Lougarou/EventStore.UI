...
CollapsibleTree = function(d3, moment,div_id, scope, msg){
    'use strict';
    ...
    function update(source) {
        ...
        var nodeEnter = node.enter().append('g')
            ...
            .on('mouseover', function(d){
                ...
                .attr('x1', function(){ return y;})
                .attr('y1', function(){ return x+7;})
                .attr('x2', function(){ return y;})
                .attr('y2', function(){ return height+25;})
                .attr('stroke','gray')
                .attr('stroke-dasharray','2,2')
                .attr('stroke-width','1');

                d.droplineText = svg.append('text')
                .attr('x', function() { return y; })
                .attr('y', function() { return height+70; })
                .attr('text-anchor', 'middle')
                .attr('font-weight','bold')
                .text(function() { return new moment(d.timestamp).format('HH:mm:ss SSS'); });
            })
        ...
        nodeEnter.append('text') //node name
            ...
            .text(function(d) {
                ...
                if(d.name.length > maxLength){
                    return truncateString(d.name, maxLength);
                }
                else{
                    return d.name;
                }
        ...
        var diagonal = function(d){
            var result = '';
            result += 'M' + d.source.y + ',' + d.source.x;
            result += 'C' + d.source.y +  ',' + (d.source.x + d.target.x) / 2;
            result += ' ' + d.target.y + ',' + (d.source.x + d.target.x) / 2;
            result += ' ' + d.target.y + ',' + d.target.x;
            return result;
        };
    ...
    function smartCollapse(){
        ...
        var smartCollapseInternal = function(){
            var coords = [];
            var maxTime = -1;
    ...
    this.draw = function(){
        ...
        root.x0 = height / 2;
        root.y0 = 0;
        root.prevTimeScale = function(){return 0;};
    ...
    this.addEvents = function(events, firstBatch){
        if(firstBatch){
            for(var i in events){
                rootPosition = Math.min(rootPosition, events[i].positionEventNumber);
            }
        }
